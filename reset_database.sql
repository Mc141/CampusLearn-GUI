-- CampusLearn™ Database Reset Script
-- This script will completely reset the database by dropping all objects and recreating them
...
-- Drop views first
DROP VIEW IF EXISTS question_details CASCADE;
DROP VIEW IF EXISTS topic_details CASCADE;
...
DROP TRIGGER IF EXISTS update_tutor_matching_requests_updated_at ON tutor_matching_requests;
...
DROP TABLE IF EXISTS tutor_matching_requests CASCADE;
DROP TABLE IF EXISTS tutor_application_modules CASCADE;
...
DROP TABLE IF EXISTS topic_tutors CASCADE;
...
DO $$
BEGIN
    RAISE NOTICE 'CampusLearn™ database has been successfully reset!';
    RAISE NOTICE 'All tables, indexes, triggers, views, and functions have been recreated.';
