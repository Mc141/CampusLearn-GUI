...
interface TopicAnswerAttachmentsProps {
  answerId: string;
  canDelete?: boolean;
}

const TopicAnswerAttachments: React.FC<TopicAnswerAttachmentsProps> = ({
  answerId,
  canDelete = true,
}) => {
  const { user } = useAuth();
  const [attachments, setAttachments] = useState<TopicAnswerAttachment[]>([]);
  ...
  const loadAttachments = async () => {
    try {
      ...
      const fetchedAttachments =
        await topicAnswerAttachmentService.getAttachmentsForAnswer(answerId);
      setAttachments(fetchedAttachments);
    } catch (err) {
      console.error("Error loading answer attachments:", err);
  ...
  const handleDownload = async (attachment: TopicAnswerAttachment) => {
    try {
      await topicAnswerAttachmentService.incrementDownloadCount(attachment.id);
  ...
  const handleDeleteAttachment = async (attachmentId: string) => {
    try {
      await topicAnswerAttachmentService.deleteAttachment(attachmentId);
  ...
  const canDeleteAttachment = (attachment: TopicAnswerAttachment) => {
    return (
      user &&
      canDelete &&
      (user.role === "admin" ||
...
export default TopicAnswerAttachments;
