...
const ForumFileUpload: React.FC<ForumFileUploadProps> = ({
  ...
  const onDrop = useCallback(
    async (acceptedFiles: File[]) => {
      ...
      // If we have a postId or replyId, upload immediately
      if (postId || replyId) {
        setUploading(true);
        setError(null);
        setUploadProgress([]);
        ...
        // If no postId/replyId, just pass files to callback (for post creation)
        onFilesUploaded?.(acceptedFiles);
        setShowUploadDialog(false);
      }
    },
    [postId, replyId, userId, onFilesUploaded]
  );
